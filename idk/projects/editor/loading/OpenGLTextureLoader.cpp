...
namespace idk
{
	...
	ResourceBundle OpenGLTextureLoader::Load_DDS(PathHandle path_to_resource)
	{
		...
		DDS_HEADER header;
		auto texture_handle = Core::GetResourceManager().LoaderEmplaceResource<ogl::OpenGLTexture>(true);
		auto tm = texture_handle->GetMeta();
	...
	ResourceBundle OpenGLTextureLoader::Load_DDS(PathHandle path_to_resource, const MetaBundle& path_to_meta)
	{
		...
		auto texture_handle = Core::GetResourceManager().LoaderEmplaceResource<ogl::OpenGLTexture>(metadata.guid, true);
