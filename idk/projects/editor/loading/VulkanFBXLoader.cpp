...
namespace idk
{
	...
	FileResources VulkanFBXLoader::Create(FileHandle path_to_resource)
	{
		...
		// So that we dont call the default constructed mesh
		//auto mesh_handle = Core::GetResourceManager().Create<Mesh>(path_to_resource);
		//auto& vkn_mesh = mesh_handle.as<vkn::VulkanMesh>();
