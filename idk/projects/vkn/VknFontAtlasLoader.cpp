...
namespace idk::vkn
{
	...
	vk::SamplerAddressMode GetRepeatMode(FontAtlasOptions options, [[maybe_unused]] UvAxis axis)
	{
		auto repeat_mode = options.uv_mode;
		vk::SamplerAddressMode mode = vk::SamplerAddressMode::eClampToEdge;
		static const auto& map = UVModeMap();
	...
	size_t ComputeFontAtlasLength(size_t width, size_t height, vk::Format format)
	{
	...
	namespace vfa
	{
		...
		/*FACreateInfo ColorBufferTexInfo(uint32_t width, uint32_t height)
		{
			FACreateInfo info{};
			info.width = width;
			info.height = height;
		...
		FACreateInfo DepthBufferTexInfo(uint32_t width, uint32_t height)
		{
			FACreateInfo info{};
			info.width = width;
			info.height = height;
	...
	void FontAtlasLoader::LoadFontAtlas(VknFontAtlas& texture, hlp::MemoryAllocator& allocator, vk::Fence load_fence, std::optional<FontAtlasOptions> ooptions, const FACreateInfo& load_info, std::optional<InputFAInfo> in_info)
	{
	...
	void FontAtlasLoader::LoadFontAtlas(VknFontAtlas& texture, TextureFormat pixel_format, std::optional<FontAtlasOptions> ooptions, const char* rgba32, size_t len, ivec2 size, hlp::MemoryAllocator& allocator, vk::Fence load_fence, bool isRenderTarget)
	{
		FontAtlasOptions options{};
		if (ooptions)
			options = *ooptions;
	...
	void FontAtlasLoader::LoadFontAtlas(VknFontAtlas& texture, TextureFormat pixel_format, std::optional<FontAtlasOptions> options, string_view rgba32, ivec2 size, hlp::MemoryAllocator& allocator, vk::Fence load_fence, bool isRenderTarget)
	{
	...
	FontAtlasResult LoadFontAtlas(hlp::MemoryAllocator& allocator, vk::Fence fence, const FACreateInfo& load_info, std::optional<InputFAInfo> in_info)
	{
		...
		if (!in_info) { //If data isn't given.
			len = ComputeFontAtlasLength(width, height, format);
		}
		else
		{
			len = in_info->len;
		...
		if (in_info)
		{
			...
			{
				auto&& [img, al] = vfa::CreateBlitImage(allocator, load_info.mipmap_level, width, height, format);
			...
			vfa::TransitionImageLayout(cmd_buffer, src_flags, src_stages, dst_flags, dst_stages, vk::ImageLayout::eUndefined, next_layout, copy_dest, img_aspect, sub_range);
			//if (!is_render_target)
			{
			...
			if (internal_format != format)
			{
				vfa::TransitionImageLayout(cmd_buffer, vk::AccessFlagBits::eTransferWrite, vk::PipelineStageFlagBits::eTransfer, vk::AccessFlagBits::eTransferRead, vk::PipelineStageFlagBits::eTransfer, vk::ImageLayout::eTransferDstOptimal, vk::ImageLayout::eTransferSrcOptimal, copy_dest, img_aspect, sub_range);
				vfa::TransitionImageLayout(cmd_buffer, {}, vk::PipelineStageFlagBits::eAllCommands, vk::AccessFlagBits::eTransferWrite, vk::PipelineStageFlagBits::eTransfer, vk::ImageLayout::eUndefined, vk::ImageLayout::eTransferDstOptimal, *image, img_aspect, sub_range);
				vfa::BlitConvert(cmd_buffer, img_aspect, *blit_src_img, *image, load_info.mipmap_level, width, height);
			}
			vfa::TransitionImageLayout(cmd_buffer, vk::AccessFlagBits::eTransferWrite, vk::PipelineStageFlagBits::eTransfer, dst_flags, dst_stages, vk::ImageLayout::eTransferDstOptimal, load_info.layout, *image, img_aspect);


		}
		else
		{
			vfa::TransitionImageLayout(cmd_buffer, vk::AccessFlagBits::eTransferWrite, vk::PipelineStageFlagBits::eTransfer, {}, vk::PipelineStageFlagBits::eAllCommands, vk::ImageLayout::eUndefined, load_info.layout, *image, img_aspect);
		}
	...
	FontAtlasResult LoadFontAtlas(hlp::MemoryAllocator& allocator, vk::Fence fence, const void* data, uint32_t width, uint32_t height, size_t len, vk::Format format, bool is_render_target)
	{
		FontAtlasResult result;
		VulkanView& view = Core::GetSystem<VulkanWin32GraphicsSystem>().Instance().View();
		vk::PhysicalDevice pd = view.PDevice();
