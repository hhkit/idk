...
////// 
	...
	//
	//		fontAtlas->char_map[i].advance.x = s_cast<float>(glyph->advance.x >> 6);
	//		fontAtlas->char_map[i].advance.y = s_cast<float>(glyph->advance.y >> 6);
	//
	//		fontAtlas->char_map[i].glyph_size.x = (float)glyph->bitmap.width;
	//		fontAtlas->char_map[i].glyph_size.y = (float)glyph->bitmap.rows;
	//
	//		fontAtlas->char_map[i].bearing.x = (float)glyph->bitmap_left;
	//		fontAtlas->char_map[i].bearing.y = (float)glyph->bitmap_top;
	//
	//		fontAtlas->char_map[i].tex_offset.x = x / (float)w;
	//		fontAtlas->char_map[i].tex_offset.y = y / (float)h;
	//
	//		mh = std::max(mh, glyph->bitmap.rows);
	//		//mh = g->bitmap.rows;
