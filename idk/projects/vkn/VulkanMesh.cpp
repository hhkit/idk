...
namespace idk::vkn
{
	VulkanMesh::VulkanMesh(const CompiledMesh& m)
	{
		...
		mm.SetIndexBuffer16(*this, mm.CreateBuffer(string_view{ r_cast<const char*>(std::data(m.element_buffer)),hlp::buffer_size(m.element_buffer) }),m.element_buffer.size());
	}
	//#pragma optimize("",off)
