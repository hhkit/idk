	...
	void MonoBehaviorEnvironment::FindMonoBehaviors()
	{
		...
		for (int i = 0; i < rows; ++i)
		{
			...
			if (is_monobehavior)
			{
				...
				find_method(type, "OnLobbyCreated", 1);
				find_method(type, "OnLobbyJoined", 1);
				find_method(type, "OnLobbyMemberJoined", 1);
				find_method(type, "OnLobbyMemberLeft", 1);
				...
				find_method(type, "OnClientDisconnected", 1);
