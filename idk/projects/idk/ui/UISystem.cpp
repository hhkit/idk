...
namespace idk
{
    void UISystem::LateInit()
    {
        ...
        auto mat = Core::GetResourceManager().Create<Material>(Guid{ 0x90da4f5c, 0x0453, 0x4e77, 0xbb3fb506c067d085 });
        mat->_shader_program = frag;
        mat->Name("Default UI");
        auto inst = Core::GetResourceManager().Create<MaterialInstance>(UISystem::default_material_inst);
        inst->material = mat;
