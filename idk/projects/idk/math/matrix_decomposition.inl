...
namespace idk
{
	...
	quaternion<T> decompose_rotation_matrix(const tmat<T, 4, 4>& rotation)
	{
		...
		if (tr > 0)
		{
			...
			retval.z = (rotation[1][0] - rotation[0][1]) / S;
		}
		else if ((rotation[0][0] > rotation[1][1]) & (rotation[0][0] > rotation[2][2]))
		{
			float S = sqrt(_1 + rotation[0][0] - rotation[1][1] - rotation[2][2]) * _2; // S=4*qx 
			retval.w = (rotation[2][1] - rotation[1][2]) / S;
			retval.x = _0_25 * S;
			retval.y = (rotation[0][1] + rotation[1][0]) / S;
			retval.z = (rotation[0][2] + rotation[2][0]) / S;
		}
		else if (rotation[1][1] > rotation[2][2]) 
		{
			float S = sqrt(_1 + rotation[1][1] - rotation[0][0] - rotation[2][2]) * _2; // S=4*qy
			retval.w = (rotation[0][2] - rotation[2][0]) / S;
			retval.x = (rotation[0][1] + rotation[1][0]) / S;
			retval.y = _0_25 * S;
			retval.z = (rotation[1][2] + rotation[2][1]) / S;
		}
		else 
		{
			float S = sqrt(_1 + rotation[2][2] - rotation[0][0] - rotation[1][1]) * _2; // S=4*qz
			retval.w = (rotation[1][0] - rotation[0][1]) / S;
			retval.x = (rotation[0][2] + rotation[2][0]) / S;
			retval.y = (rotation[1][2] + rotation[2][1]) / S;
			retval.z = _0_25 * S;
		}
